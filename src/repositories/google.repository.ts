...
async function readBrazueraoSheet() {
  try {    
    let betBrazueraoInfoUsers : IBetBrazueraoInfoUser[] = localStorageService.getItem(LocalStorageKeysCache.GOOGLE_SHEET_SERVICE_GET_BRAZUERAO);

    if (betBrazueraoInfoUsers && betBrazueraoInfoUsers.length > 0)
      return betBrazueraoInfoUsers;

    const { data : brazueraoSheet} = await apiGoogleSheet.get<sheets_v4.Schema$ValueRange>('/api/sheets');

    if (!brazueraoSheet)
      throw ('Planilha não encontrada!');

    const rows = brazueraoSheet.values;
    if (!rows || rows.length === 0)
      throw ('Planilha vazia!');
    ...
    betBrazueraoInfoUsers = await Promise.all(
      betBrazueraoInfoUsers.map(async (betBrazueraoInfoUser) =>{
        ...
        betBrazueraoInfoUser.teamsClassification = await formatTeamsNames(teamsClassification);
...
async function formatTeamsNames(teamsNames: string[]) : Promise<string[]> {
  return await Promise.all(teamsNames.map(async teamName => await formatTeamName(teamName)));
}
...
async function formatTeamName(nameTeam: string){
  try {
    ...
    if (nameTeam.toLocaleLowerCase().includes('galo'))
      ...
      formattedNameTeam = teams.find(team => nameTeam.split(' ').every(nameTeam => team.toLowerCase().includes(nameTeam.toLocaleLowerCase())));
