...
async function getBrasileiraoTeamsNames() : Promise<string[] | undefined> {
  try {
    ...
    if (!leagueTable)
      throw new Error("Times do Brasileirão não encontrados.")
...
async function getBrasileiraoTable() : Promise<ITeamPositionInfo[]> {
  try {
    let leagueTableData : ITeamPositionInfo[] = localStorageService.getItem(LocalStorageKeysCache.BRASILEIRAO_LEAGUE_SERVICE_GET_TABLE);
...
async function getBrasilianLeague() {
  ...
  try {
    ...
    console.error(`Error in API Futebol. Erro message: ${(error as Error).message}`)
...
async function getBrasilianLeagueGE() {
  try {
    const { data : brasilianLeagueApiGE} = await apiBrasileiraoGloboEsporte.get<any[]>('/api/brasileirao');
    ...
    console.error(`Error in API Brasileirao GloboEsporte. Erro message: ${(error as Error).message}`)
    const errorMessage = `API Brasileirao GloboEsporte: ${(error as Error).message}`;
    throw new Error(errorMessage);
  }  
}
...
async function getBrasilianLeagueApiFutebol() {
  try {
    const { data : brasilianLeagueApiFutebol} = await apiFutebol.get<ITeamPositionApiInfo[]>('/v1/campeonatos/10/tabela');
