...
apiBrasileiraoGloboEsporte.interceptors.request.use(function (config) {
...
async function handler(
  ...
  try {
    ...
    const { data } = await apiBrasileiraoGloboEsporte.get('/futebol/brasileirao-serie-a', {responseType: 'document'})
    ...
    console.error(error);
    res.status(500).json({ message: 'Internal Error', Error: (error as Error).message });
  }
}
